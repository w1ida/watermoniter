...
<div class="box">
            ...
            <div class="col-md-6">
              <div class="">
                <select class="form-control select2" multiple="multiple" id="select_areas" data-placeholder="选择一个地点" style="width: 60%;">
					...
					<option value="{{$waterarea->aid}}">{{$waterarea->aname}}</option>
					@endforeach
                </select>
	...
	<div class="box-body">
	  ...
	  <table class="table table-bordered">
		...
		<tr>
		  <td>{{$point->pid}}</td>
			<td><a href="/admin/data/{{$point->pid}}">{{$point->pname}}</a></td>
			<td><a href="/admin/monitorpoint?aids=%5B{{$point->aid}}%5D">{{$point->area->aname}}</a></td>
		  <td>{{$point->lng}},{{$point->lat}}</td>
		  <td style="min-width:130px">

			<div class="btn-group">
                <button type="button" class="btn btn-xs btn-success" onclick="location.href='/admin/data/analysis/{{$point->pid}}'">分析</button>
			  <button type="button" class="btn btn-xs btn-info editpoint" data-pointid="{{ $point->pid }}"><i class="fa fa-edit"></i>编辑</button>
			  ...
			  <ul class="dropdown-menu " style="min-width:0px;" role="menu">
				  <li><a class="text-red delpoint" href="#" data-pointid="{{ $point->pid }}" ><i class="fa fa-remove"></i>删除</a></li>
			  </ul>
    ...
    var addlayerid=layer.open({
        ...
        success :function(e){
            ...
            $('#adduser_do').click(function() {
                $.post(location.pathname,{_method:'post',_token:'{{csrf_token()}}',data:{
                    pname:$('#add_pname').val(),
                    aid:$('#add_aid').val(),
                    lng:$('#add_lng').val(),
                    lat:$('#add_lat').val(),
            ...
            layer.open({
                ...
                success :function(){
                    ...
                    $('#edit_do').click(function() {
                        $.post(location.pathname+'/'+pointId,{_method:'put',_token:'{{csrf_token()}}',data:{
