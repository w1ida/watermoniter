...
<div class="box">
	...
	<div class="box-body">
	  <table class="table table-bordered">
		...
		<tr>
		  <td>{{ $waterarea->aid }}</td>
			<td><a href="/admin/monitorpoint?aids=%5B{{$waterarea->aid}}%5D">{{ $waterarea->aname }}</a></td>
		  <td>{{ $waterarea->remark }}</td>
		  <td>{{ $waterarea->created_at }}</td>
		  <td>
			<div class="btn-group">
			  <button type="button" class="btn btn-xs btn-info editarea" data-areaid="{{ $waterarea->aid }}"><i class="fa fa-edit"></i>编辑</button>
			  <button type="button" class="btn btn-info btn-xs dropdown-toggle" data-toggle="dropdown">
				<span class="caret"></span>
				<span class="sr-only">Toggle Dropdown</span>
			  </button>
			  <ul class="dropdown-menu " style="min-width:0px;" role="menu">
				  <li><a class="text-red delarea" href="#" data-areaid="{{ $waterarea->aid }}" ><i class="fa fa-remove"></i>删除</a></li>
			  </ul>
			</div>
		  </td>
		</tr>
            ...
            var addlayerid=layer.open({
                ...
                success :function(e){
                    ...
                    $('#adduser_do').click(function() {
                        $.post(location.pathname,{_method:'post',_token:'{{csrf_token()}}',data:{aname:$('#add_aname').val(),
                ...
                layer.open({
                    ...
                    success :function(){
                        ...
                        $('#edit_do').click(function() {
                            $.post(location.pathname+'/'+areaId,{_method:'put',_token:'{{csrf_token()}}',data:{aname:$('#edit_aname').val(),
